...
const DashboardMapTabs: React.FC<DashboardMapTabsProps> = ({
  ...
  // Make refresh function refs so we don't depend on their identity in effects
  ...
  // Optionally: call each refresh only once when the tab first becomes active
  const fetchedOnceRef = useRef<{ ideas: boolean; issues: boolean }>({ ideas: false, issues: false });
  useEffect(() => {
    if (active === "ideas" && refreshIdeasRef.current && !fetchedOnceRef.current.ideas) {
      fetchedOnceRef.current.ideas = true;
      void refreshIdeasRef.current();
    } else if (active === "issues" && refreshIssuesRef.current && !fetchedOnceRef.current.issues) {
      fetchedOnceRef.current.issues = true;
      void refreshIssuesRef.current();
    }
  }, [active]);
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      ...
      <div className="relative h-[420px] w-full overflow-hidden rounded-lg border">
        ...
        {active === "ideas" && (
          <IdeaMapViewerDashboard ideas={ideas ?? []} refreshIdeas={refreshIdeasRef.current || (() => {})} />
        )}
        {active === "issues" && refreshIssuesRef.current && (
          <ReportIssueMapViewerDashboard user={user} issues={filteredIssues ?? []} refreshIssues={() => {}} />
        )}
      </div>
    </div>
  );
